...
object SimpleTestHanjin {
  ...
  def main(args: Array[String]) {
   ...
   subject.addObserver(new MarkingObserver())
    ...
    case class BindTransition(x:Option[Int], y:Option[String], c:Option[Int])
    
    case class BindTransInit(x:Option[Int])
    
    case class BindTransProcess(x:Option[Int], y:Option[String])
    ...
    // x (int) output arc for init to start
    val arcExpArc3:(colset_CASEID => Option[colset_CASEID]) = (inp:Any) => inp match { case(x:colset_CASEID) => { Some(x+1) } } //arc1 exp
    ...
    val addTimeArc3:(BindTransInit => Long) = (b:BindTransInit) => { Math.round(18.44) }
    ...
    //PROCESS
    //---------------------- place 2 tmp1 ------------------
    val ms2tmp1 = new Multiset[colset_CASEIDxRES](Map[(colset_CASEIDxRES,Long),Int]())
    val pplace2tmp1 = new Place("p2tmp1","place 2 tmp1",ms2tmp1)
    
    //---------------------- place 2 tmp2 ------------------
    val ms2tmp2 = new Multiset[colset_CASEIDxRES](Map[(colset_CASEIDxRES,Long),Int]())
    ...
    //---------------------- place 3 tmp ------------------
    val ms3tmp = new Multiset[colset_CASEIDxRES](Map[(colset_CASEIDxRES,Long),Int]())
    val pplace3tmp = new Place("p3tmp","place 3 tmp",ms3tmp)
    
    //---------------------- place 4 ------------------
    val ms4 = new Multiset[colset_CASEID](Map[(colset_CASEID,Long),Int]())
    val pplace4 = new Place("p4","place 4",ms4)
    
    //---------------------- place 4 tmp ------------------
    val ms4tmp = new Multiset[colset_CASEIDxRES](Map[(colset_CASEIDxRES,Long),Int]())
    val pplace4tmp = new Place("p4tmp","place 4 tmp",ms4tmp)
    
    //---------------------- place 5 ------------------
    val ms5 = new Multiset[colset_CASEID](Map[(colset_CASEID,Long),Int]())
    val pplace5 = new Place("p5","place 5",ms5)
    
    //---------------------- place 5 tmp ------------------
    val ms5tmp = new Multiset[colset_CASEIDxRES](Map[(colset_CASEIDxRES,Long),Int]())
    val pplace5tmp = new Place("p5tmp","place 5 tmp",ms5tmp)
    
    //---------------------- place 6 ------------------
    val ms6 = new Multiset[colset_CASEID](Map[(colset_CASEID,Long),Int]())
    val pplace6 = new Place("p6","place 6",ms6)
    
    //---------------------- place 6 tmp ------------------
    val ms6tmp = new Multiset[colset_CASEIDxRES](Map[(colset_CASEIDxRES,Long),Int]())
    val pplace6tmp = new Place("p6tmp","place 6 tmp",ms6tmp)
    
    //---------------------- place 7 ------------------
    val ms7 = new Multiset[colset_CASEID](Map[(colset_CASEID,Long),Int]())
    ...
    val arcExpArc4_st:(colset_CASEID => Option[colset_CASEID]) = (x:colset_CASEID) => { Some(x) } //arc4_st exp
    val TtoBArc4_st:(colset_CASEID => BindTransProcess) = (inp:Any) => BindTransProcess(inp match { case x:Int => Some(x); case _ => None }, None)
    val BtoTArc4_st:(BindTransProcess => colset_CASEID) = (b:BindTransProcess) => { b.x.get }
    ...
    val arcExpArc4_cptmp:(colset_CASEIDxRES => Option[colset_CASEIDxRES]) = (inp:Any) => inp match { case (x:Int,y:String) => { Some(x,y)  } } //arc4_cptmp exp
    val TtoBArc4_cptmp:(colset_CASEIDxRES => BindTransProcess) = (inp:Any) => BindTransProcess(inp match { case (x:Int,y:Any) => Some(x); case _ => None }, inp match { case (x:Any,y:String) => Some(y); case _ => None })
    val BtoTArc4_cptmp:(BindTransProcess => colset_CASEIDxRES) = (b:BindTransProcess) => { (b.x.get, b.y.get) }
    ...
    val arcExpArc4_sttmp = (inp:Any) => inp match { case (x:Int,y:String) => { Some(x,y)  } } //arc2 exp
    ...
    val BtoTArc4_sttmp = (b:BindTransProcess) => { (b.x.get,b.y.get) }
    ...
    val arcExpArc4_stres = (inp:Any) => inp match { case y:String => { if(filterResGroup1(y)) Some(y) else None  } } //arc2 exp
    val TtoBArc4_stres = (inp:Any) => BindTransProcess(None, inp match { case y:String => Some(y); case _ => None })
    val BtoTArc4_stres = (b:BindTransProcess) => { b.y.get }
    ...
    val arcExpArc4_cpres = (inp:Any) => inp match { case y:String => { if(filterResGroup1(y)) Some(y) else None  } } //arc2 exp
    val TtoBArc4_cpres = (inp:Any) => BindTransProcess(None, inp match { case y:String => Some(y); case _ => None })
    val BtoTArc4_cpres = (b:BindTransProcess) => { b.y.get }
    ...
    val arcExpArc4_cp = (inp:Any) => inp match { case x:Int => { Some(x)  } } //arc2 exp
    val TtoBArc4_cp = (inp:Any) => BindTransProcess(inp match { case x:Int => Some(x); case _ => None }, None)
    val BtoTArc4_cp = (b:BindTransProcess) => { b.x.get }
    ...
    val arcExpArc5_st = (inp:Any) => inp match { case x:Int => { Some(x) } } //arc5 exp
    val TtoBArc5_st = (inp:Any) => BindTransProcess(inp match { case x:Int => Some(x); case _ => None }, None)
    val BtoTArc5_st = (b:BindTransProcess) => { b.x.get }
    ...
    val arcExpArc5_cptmp = (inp:Any) => inp match { case (x:Int,y:String) => { Some(x,y)  } } //arc5_cptmp exp
    ...
    val BtoTArc5_cptmp = (b:BindTransProcess) => { (b.x.get,b.y.get) }
    val arc5_cptmp = new Arc[colset_CASEIDxRES,BindTransProcess]("arc5cptmp", pplace2tmp2, ttrans3_start, Direction.TtP)
    val addTimeArc5_cptmp = (inp:Any) => inp match { case inp:BindTransProcess => { Math.round(Gaussian(100, 10).draw()) } }
    ...
    val arcExpArc5_sttmp = (inp:Any) => inp match { case (x:Int,y:String) => { Some(x,y)  } } //arc2 exp
    ...
    val BtoTArc5_sttmp = (b:BindTransProcess) => { (b.x.get,b.y.get) }
    ...
    val arcExpArc5_stres = (inp:Any) => inp match { case y:String => { if(filterResGroup2(y)) Some(y) else None  } } //arc2 exp
    val TtoBArc5_stres = (inp:Any) => BindTransProcess(None, inp match { case y:String => Some(y); case _ => None })
    val BtoTArc5_stres = (b:BindTransProcess) => { b.y.get }
    ...
    val arcExpArc5_cpres = (inp:Any) => inp match { case y:String => { if(filterResGroup2(y)) Some(y) else None  } } //arc2 exp
    ...
    val arcExpArc5_cp = (inp:Any) => inp match { case x:Int => { Some(x) } } //arc2 exp
    ...
    val arcExpArc6_st = (inp:Any) => inp match { case x:Int => { Some(x) } } //arc1 exp
    ...
    val arcExpArc6_cptmp = (inp:Any) => inp match { case (x:Int,y:String) => { Some(x,y) } } //arc2 exp
    ...
    val arcExpArc6_sttmp = (inp:Any) => inp match { case (x:Int,y:String) => { Some(x,y) } } //arc2 exp
    ...
    val arcExpArc6_stres = (inp:Any) => inp match { case y:String => { if(filterResGroup3(y)) Some(y) else None  } } //arc2 exp
    ...
    val arcExpArc6_cpres = (inp:Any) => inp match { case y:String => { if(filterResGroup3(y)) Some(y) else None  } } //arc2 exp
    ...
    val arcExpArc6_cp = (inp:Any) => inp match { case x:Int => { Some(x) } } //arc2 exp
    ...
    val arcExpArc7_st = (inp:Any) => inp match { case x:Int => { Some(x) } } //arc1 exp
    ...
    // (x,y) (int,string) output arc from ds_quayside_start to place2tmp1
    val arcExpArc7_cptmp = (inp:Any) => inp match { case (x:Int,y:String) => { Some(x,y) } } //arc2 exp
    ...
    // (x,y) (int,string) output arc from place2tmp1 to ds_quayside_complete
    val arcExpArc7_sttmp = (inp:Any) => inp match { case (x:Int,y:String) => { Some(x,y) } } //arc2 exp
    ...
    // y (string) input arc from resource to ds_quayside_start
    val arcExpArc7_stres = (inp:Any) => inp match { case y:String => { if(filterResGroup3(y)) Some(y) else None  } } //arc2 exp
    ...
    // y (string) input arc from ds_quayside_complete to resource
    val arcExpArc7_cpres = (inp:Any) => inp match { case y:String => { if(filterResGroup3(y)) Some(y) else None  } } //arc2 exp
    ...
    // x (int) output arc from ds_quayside_comp to place3
    val arcExpArc7_cp = (inp:Any) => inp match { case x:Int => { Some(x) } } //arc2 exp
    ...
    val arcExpArc8_st = (inp:Any) => inp match { case x:Int => { Some(x) } } //arc1 exp
    ...
    val arcExpArc8_cptmp = (inp:Any) => inp match { case (x:Int,y:String) => { Some(x,y) } } //arc2 exp
    ...
    val arcExpArc8_sttmp = (inp:Any) => inp match { case (x:Int,y:String) => { Some(x,y) } } //arc2 exp
    ...
    val arcExpArc8_stres = (inp:Any) => inp match { case y:String => { if(filterResGroup2(y)) Some(y) else None  } } //arc2 exp
    ...
    val arcExpArc8_cpres = (inp:Any) => inp match { case y:String => { if(filterResGroup2(y)) Some(y) else None  } } //arc2 exp
    ...
    val arcExpArc8_cp = (inp:Any) => inp match { case x:Int => { Some(x) } } //arc2 exp
    ...
    val arcExpArc9_st = (inp:Any) => inp match { case x:Int => { Some(x) } } //arc1 exp
    ...
    // (x,y) (int,string) output arc from ds_quayside_start to place2tmp1
    val arcExpArc9_cptmp = (inp:Any) => inp match { case (x:Int,y:String) => { Some(x,y) } } //arc2 exp
    ...
    // (x,y) (int,string) output arc from place2tmp1 to ds_quayside_complete
    val arcExpArc9_sttmp = (inp:Any) => inp match { case (x:Int,y:String) => { Some(x,y) } } //arc2 exp
    ...
    // y (string) input arc from resource to ds_quayside_start
    val arcExpArc9_stres = (inp:Any) => inp match { case y:String => { if(filterResGroup1(y)) Some(y) else None  } } //arc2 exp
    ...
    // y (string) input arc from ds_quayside_complete to resource
    val arcExpArc9_cpres = (inp:Any) => inp match { case y:String => { if(filterResGroup1(y)) Some(y) else None  } } //arc2 exp
    ...
    // x (int) output arc from ds_quayside_comp to place3
    val arcExpArc9_cp = (inp:Any) => inp match { case x:Int => { Some(x) } } //arc2 exp
