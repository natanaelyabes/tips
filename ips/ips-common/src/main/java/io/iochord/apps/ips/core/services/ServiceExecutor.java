...
public class ServiceExecutor {
	...
	@Async(NAME)
	public <C, R> CompletableFuture<ServiceContext> runAsync(ServiceContext context, AnIpsService<C, R> service, String jsonConfig, Class<C> configClass, Class<R> resultClass) {
		return CompletableFuture.completedFuture(run(context, service, jsonConfig, configClass, resultClass));
	}
	
	public <C, R> ServiceContext run(ServiceContext context, AnIpsService<C, R> service, String jsonConfig, Class<C> configClass, Class<R> resultClass) {
	...
	@Async(NAME)
	public <C, R> CompletableFuture<ServiceContext> runAsync(ServiceContext context, AnIpsService<C, R> service, C config, Class<R> resultClass) {
		return CompletableFuture.completedFuture(run(context, service, config, resultClass));
	}
	
	public <C, R> ServiceContext run(ServiceContext context, AnIpsService<C, R> service, C config, Class<R> resultClass) {
		try {
			context.start();
