...
export default class LoadingButton extends Vue {
  ...
  private styled: boolean = false;
  private timelapse: number = 2000;

  public mounted() {
    console.info(this.loading);
    console.info(this.styled);
  }

  public click() {
    this.toggle(); setTimeout(() => this.toggle(), this.timelapse);
  }

  public toggle() {
    this.loading = this.loading === false ? true : false;
    this.styled = this.styled === false ? true : false;
  }
}
</script>