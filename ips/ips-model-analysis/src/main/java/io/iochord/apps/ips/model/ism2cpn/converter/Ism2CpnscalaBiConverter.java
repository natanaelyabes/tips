			...
			for (String ni : p.getNodes().keySet()) {
				...
				if (n instanceof Activity) {
					...
					if(na.getResource() != null) {
						   ...
						   String p_nap3 = addPlace("_start_0", "_nap3", entTypeId, "", na.getId());
						   ...
						   String p_nap3 = addPlace("_start_0", na.getLabel() + "_nap3", entTypeId, "", na.getId());
				...
				if (n instanceof Branch) {
					...
					for (Connector c : p.getConnectors().values()) {
						...
						String p_bpis = addPlace(b.getId()+"_end_"+i, "_bpi_" + i + "s", entTypeId, "", b.getId());
