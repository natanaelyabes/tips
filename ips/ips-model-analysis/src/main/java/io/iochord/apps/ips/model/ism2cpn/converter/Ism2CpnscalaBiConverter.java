...
public class Ism2CpnscalaBiConverter implements Converter<IsmGraph, Ism2CpnscalaModel> {
	...
	public String addPlace(String hubid, String name, String type, String initialMarking, String origin,
		...
		placefactory.append(
				"val " + placeidClean + " = new Place(\"" + placeidClean + "\",\"" + name + "\"," + multisetid + ")\n");
		placefactory.append(placeidClean + ".setOrigin(Map[String,String]((\"origin\",\"" + origin + "\"),(\"role\",\""
				+ name + "\")))\n");
	...
	public String addTransition(String name, String guard, String action, String classbinding, String eval,
			String merge, String origin, String transId) {
		...
		transfactory.append("val " + transidClean + " = new Transition[" + classbinding + "](\"" + transidClean
				+ "\",\"" + name + "\"," + guard + "," + action + ")\n");
		transfactory.append(transidClean + ".setEval(" + eval + ")\n");
		transfactory.append(transidClean + ".setMerge(" + merge + ")\n");
		transfactory.append(transidClean + ".setOrigin(Map[String,String]((\"origin\",\"" + origin + "\"),(\"role\",\""
				+ name + "\")))\n");
	...
	public String addArc(String placeid, String transitionid, String direction, String type, String classbinding,
			String TtB, String BtT, String addTime, String noToken, boolean isBase, String origin) {
		String counter = getCounter(KeyElement.arc);
		String arcid = "arc" + counter;

		StringBuilder arcfactory = new StringBuilder();
		arcfactory.append("val " + arcid + " = new Arc[" + type + "," + classbinding + "](\"" + arcid + "\"," + placeid
				+ "," + transitionid + ",Direction." + direction + ")\n");
	...
	public String addTtB(String classbinding, String type, String inverse, String TtBdef) {
		...
		StringBuilder TtBfactory = new StringBuilder();
		TtBfactory.append("val " + TtBid + " = (t:" + type + ") => { try { val " + inverse + " = t;Some(" + classbinding
				+ "(" + TtBdef + ")) } catch { case e: Exception => None } }\n");
	...
	public String createResourcePlace(String resId, String typecaseId) {
		...
		placefactory.append(
				"val ms" + residClean + " = new Multiset[Resource[" + typecaseId + "]](map" + residClean + ")\n");
		placefactory.append(
				"val " + residClean + " = new Place(\"" + residClean + "\",\"Resource Place\",ms" + residClean + ")\n");
		placefactory.append(
				residClean + ".setOrigin(Map[String,String]((\"origin\",\"" + resId + "\"),(\"role\",\"_resp\")))\n");
	...
	public void addResource(String resId, String name, int numbofresource) {
		...
		for (int i = 1; i <= numbofresource; i++) {
			...
			resfactory.append(
					"val " + residInScala + " = new Resource(\"" + residInScala + "\", \"" + name + "\", 0L)\n");
	...
	public void addQueue(String typeid, String ltypeid, String cb, String ev, String mg, Queue queue, String origin) {
		...
		String pendqueue = addPlace(null, "_qendp", ltypeid, "((" + listid + ",0),1)", origin,
				queue.getId() + "_qendp");
		...
		addArc(pendqueue, tstartqueue, "PtT", ltypeid, cb, addTtB(cb, ltypeid, "queue", "None, Some(queue)"),
				addBtT(cb, ltypeid, "b.queue.get"), null, null, true, origin);

		if (queue.getType() == Queue.QUEUE_TYPE.FIFO)
			addArc(pendqueue, tstartqueue, "TtP", ltypeid, cb, null, addBtT(cb, ltypeid, "b.entity.get::b.queue.get"),
					null, null, false, origin);
		else
			addArc(pendqueue, tstartqueue, "TtP", ltypeid, cb, null,
					addBtT(cb, ltypeid, "b.queue.get:::List(b.entity.get)"), null, null, false, origin);
	}
	...
	public Ism2CpnscalaModel convert(IsmGraph snet) {
		...
		for (String pi : snet.getPages().keySet()) {
			...
			String e_entTypeId = addEval("(b1.entity == b2.entity || b1.entity == None || b2.entity == None)",
					b_entTypeId);
			String m_entTypeId = addMerge("val entity = if(b1.entity == None) b2.entity else b1.entity;", b_entTypeId,
					"entity");
			...
			String b_entResTypeId = addBindingClass(
					"entity:Option[" + entTypeId + "], resource:Option[Resource[" + entTypeId + "]]");
			String e_entResTypeId = addEval(
					"(b1.entity == b2.entity || b1.entity == None || b2.entity == None) && (b1.resource == b2.resource || b1.resource == None || b2.resource == None)",
					b_entResTypeId);
			String m_entResTypeId = addMerge(
					"val entity = if(b1.entity == None) b2.entity else b1.entity; val resource = if(b1.resource == None) b2.resource else b1.resource;",
					b_entResTypeId, "entity,resource");

			String qentResTypeId = "colset" + getCounter(KeyElement.type);
			objecttypes.put("entityResTypeId", qentResTypeId);
			factory.append("type " + qentResTypeId + " = (" + entTypeId + ",Resource[" + entTypeId + "], List["
					+ entTypeId + "])\n");
			factory.append("\n");

			String b_qentResTypeId = addBindingClass("entity:Option[" + entTypeId + "], resource:Option[Resource["
					+ entTypeId + "]], queue:Option[List[" + entTypeId + "]]");
			String e_qentResTypeId = addEval(
					...
					b_qentResTypeId);
			String m_qentResTypeId = addMerge(
					...
					b_qentResTypeId, "entity,resource,queue");
				...
				if (d instanceof Generator) {
					...
					String action = addAction(b_dgt1,
							"val r = new java.util.Random()\n" + "val rint = r.nextInt();val gid = \"" + dg.getId()
									...
									+ "(b.tid,Some(gid),Some(data))");

					String t_dgt1 = addTransition("_dgt1", guard, action, b_dgt1, e_dgt1, m_dgt1, dg.getId(), null);

					addArc(p_dgp1, t_dgt1, "PtT", typeId, b_dgt1,
							addTtB(b_dgt1, typeId, "tid", "Some(tid), None, None"), addBtT(b_dgt1, typeId, "b.tid.get"),
							null, null, true, dg.getId());
					addArc(p_dgp1, t_dgt1, "TtP", typeId, b_dgt1, null, addBtT(b_dgt1, typeId, "b.tid.get + 1"),
							addAddedTime(b_dgt1, dg.getExpression()), null, false, dg.getId());
					addArc(p_dgp2, t_dgt1, "TtP", entTypeId, b_dgt1, null,
							addBtT(b_dgt1, entTypeId, "(b.tid.get,b.gid.get)"), null, null, false, dg.getId());
					addArc(p_dgpData, t_dgt1, "TtP", dataTypeId, b_dgt1, null,
							addBtT(b_dgt1, dataTypeId, "((b.tid.get,b.gid.get),b.data.get)"), null, null, false,
							dg.getId());
			...
			for (String ni : p.getNodes().keySet()) {
				...
				if (n instanceof Start) {
					...
					if (na.getGenerator() != null) {
						...
						String t_silent = transIfExist.containsKey(t_gen) ? t_gen
								: addTransition("_start", null, null, b_entTypeId, e_entTypeId, m_entTypeId, na.getId(),
										na.getGenerator().getValue().getId() + "_mergegen");
						if (!transIfExist.containsKey(t_gen))
							addArc(placeshub.get(na.getGenerator().getId() + "_start_0"), t_silent, "PtT", entTypeId,
									b_entTypeId, addTtB(b_entTypeId, entTypeId, "entity", "Some(entity)"),
									addBtT(b_entTypeId, entTypeId, "b.entity.get"), null, null, true,
									na.getGenerator().getValue().getId());
						transIfExist.put(t_gen, true);
						addArc(pstart, t_silent, "TtP", entTypeId, b_entTypeId, null,
								addBtT(b_entTypeId, entTypeId, "b.entity.get"), null, null, false,
								na.getGenerator().getValue().getId());
					}
				}
				...
				if (n instanceof Activity) {
					...
					if (na.getType() == ActivityType.CONCURRENT_BATCH) {
						if (na.getQueue() != null && na.getResource() != null) {
							...
							addArc(p_nap1, t_qstt, "PtT", entTypeId, b_qentTypeId,
									addTtB(b_qentTypeId, entTypeId, "entity", "Some(entity), None"),
									addBtT(b_qentTypeId, entTypeId, "b.entity.get"), null, null, true, na.getId());
							String t_natstart = addTransition("_natstart", null, null, b_qentResTypeId, e_qentResTypeId,
									m_qentResTypeId, na.getId(), null);
							addArc(p_qendp, t_natstart, "PtT", qentTypeId, b_qentResTypeId,
									addTtB(b_qentResTypeId, qentTypeId, "entity::queue",
											"Some(entity), None, Some(queue)"),
									addBtT(b_qentResTypeId, qentTypeId, "b.entity.get::b.queue.get"), null, null, true,
									na.getId());
							addArc(p_qendp, t_natstart, "TtP", qentTypeId, b_qentResTypeId, null,
									addBtT(b_qentResTypeId, qentTypeId, "b.queue.get"), null, null, false, na.getId());
							String p_nap2 = addPlace(null, "_nap2", entResTypeId, "", na.getId(), null);
							addArc(p_nap2, t_natstart, "TtP", entResTypeId, b_qentResTypeId, null,
									addBtT(b_qentResTypeId, entResTypeId, "(b.entity.get,b.resource.get)"),
									addAddedTime(b_qentResTypeId, na.getProcessingTimeExpression()), null, false,
									na.getId());
							String t_natend = addTransition("_natend", null, null, b_entResTypeId, e_entResTypeId,
									m_entResTypeId, na.getId(), null);
							addArc(p_nap2, t_natend, "PtT", entResTypeId, b_entResTypeId,
									addTtB(b_entResTypeId, entResTypeId, "(entity, resource)",
											"Some(entity), Some(resource)"),
									addBtT(b_entResTypeId, entResTypeId, "(b.entity.get, b.resource.get)"), null, null,
									true, na.getId());
							String p_nap3 = addPlace(na.getId() + "_start_0", "_nap3", entTypeId, "", na.getId(), null);
							addArc(p_nap3, t_natend, "TtP", entTypeId, b_entResTypeId, null,
									addBtT(b_entResTypeId, entTypeId, "b.entity.get"), null, null, false, na.getId());

							addArc(cleanId("r_", na.getResource().getId()), t_natstart, "PtT",
									"Resource[" + entTypeId + "]", b_qentResTypeId,
									addTtB(b_qentResTypeId, "Resource[" + entTypeId + "]", "resource",
											"None, Some(resource), None"),
									addBtT(b_qentResTypeId, "Resource[" + entTypeId + "]", "b.resource.get"), null,
									null, true, na.getId());
							addArc(cleanId("r_", na.getResource().getId()), t_natend, "TtP",
									"Resource[" + entTypeId + "]", b_entResTypeId, null,
									addBtT(b_entResTypeId, "Resource[" + entTypeId + "]", "b.resource.get"), null, null,
									false, na.getId());
							...
							addArc(p_nap1, t_qstt, "PtT", entTypeId, b_qentTypeId,
									addTtB(b_qentTypeId, entTypeId, "entity", "Some(entity), None"),
									addBtT(b_qentTypeId, entTypeId, "b.entity.get"), null, null, true, na.getId());
							String t_natstart = addTransition("_natstart", null, null, b_qentTypeId, e_qentTypeId,
									m_qentTypeId, na.getId(), null);
							addArc(p_qendp, t_natstart, "PtT", qentTypeId, b_qentTypeId,
									addTtB(b_qentTypeId, qentTypeId, "entity::queue", "Some(entity), Some(queue)"),
									addBtT(b_qentTypeId, qentTypeId, "b.entity.get::b.queue.get"), null, null, true,
									na.getId());
							addArc(p_qendp, t_natstart, "TtP", qentTypeId, b_qentTypeId, null,
									addBtT(b_qentTypeId, qentTypeId, "b.queue.get"), null, null, false, na.getId());
							String p_nap2 = addPlace(null, "_nap2", entTypeId, "", na.getId(), null);
							addArc(p_nap2, t_natstart, "TtP", entTypeId, b_qentTypeId, null,
									addBtT(b_qentTypeId, entTypeId, "b.entity.get"),
									addAddedTime(b_entTypeId, na.getProcessingTimeExpression()), null, false,
									na.getId());
							String t_natend = addTransition("_natend", null, null, b_entTypeId, e_entTypeId,
									m_entTypeId, na.getId(), null);
							addArc(p_nap2, t_natend, "PtT", entTypeId, b_entTypeId,
									addTtB(b_entTypeId, entTypeId, "entity", "Some(entity)"),
									addBtT(b_entTypeId, entTypeId, "b.entity.get"), null, null, true, na.getId());
							String p_nap3 = addPlace(na.getId() + "_start_0", "_nap3", entTypeId, "", na.getId(), null);
							addArc(p_nap3, t_natend, "TtP", entTypeId, b_entTypeId, null,
									addBtT(b_entTypeId, entTypeId, "b.entity.get"), null, null, false, na.getId());
						} else if (na.getResource() != null) {
							String t_natstart = addTransition("_natstart", null, null, b_entResTypeId, e_entResTypeId,
									m_entResTypeId, na.getId(), null);
							addArc(p_nap1, t_natstart, "PtT", entTypeId, b_entResTypeId,
									addTtB(b_entResTypeId, entTypeId, "entity", "Some(entity), None"),
									addBtT(b_entResTypeId, entTypeId, "b.entity.get"), null, null, true, na.getId());
							String p_nap2 = addPlace(null, "_nap2", entResTypeId, "", na.getId(), null);
							addArc(p_nap2, t_natstart, "TtP", entResTypeId, b_entResTypeId, null,
									addBtT(b_entResTypeId, entResTypeId, "(b.entity.get,b.resource.get)"),
									addAddedTime(b_entResTypeId, na.getProcessingTimeExpression()), null, false,
									na.getId());
							String t_natend = addTransition("_natend", null, null, b_entResTypeId, e_entResTypeId,
									m_entResTypeId, na.getId(), null);
							addArc(p_nap2, t_natend, "PtT", entResTypeId, b_entResTypeId,
							...
							addArc(cleanId("r_", na.getResource().getId()), t_natstart, "PtT",
									"Resource[" + entTypeId + "]", b_entResTypeId,
									addTtB(b_entResTypeId, "Resource[" + entTypeId + "]", "resource",
											"None, Some(resource)"),
									addBtT(b_entResTypeId, "Resource[" + entTypeId + "]", "b.resource.get"), null, null,
									true, na.getId());
							addArc(cleanId("r_", na.getResource().getId()), t_natend, "TtP",
							...
							String t_natstart = addTransition("_natstart", null, null, b_entTypeId, e_entTypeId,
									m_entTypeId, na.getId(), null);
							addArc(p_nap1, t_natstart, "PtT", entTypeId, b_entTypeId,
									addTtB(b_entTypeId, entTypeId, "entity", "Some(entity)"),
									addBtT(b_entTypeId, entTypeId, "b.entity.get"), null, null, true, na.getId());
							String p_nap2 = addPlace(null, "_nap2", entTypeId, "", na.getId(), null);
							addArc(p_nap2, t_natstart, "TtP", entTypeId, b_entTypeId, null,
									addBtT(b_entTypeId, entTypeId, "b.entity.get"),
									addAddedTime(b_entTypeId, na.getProcessingTimeExpression()), null, false,
									na.getId());
							String t_natend = addTransition("_natend", null, null, b_entTypeId, e_entTypeId,
									m_entTypeId, na.getId(), null);
						...
						if (na.getQueue() != null && na.getResource() != null) {
							...
							addArc(p_nap2, t_natstart, "TtP", entResTypeId, b_qentResTypeId, null,
									addBtT(b_qentResTypeId, entResTypeId, "(b.entity.get,b.resource.get)"),
									addAddedTime(b_qentResTypeId, na.getProcessingTimeExpression()), "5", false,
									na.getId());
							String t_natend = addTransition("_natend", null, null, b_entResTypeId, e_entResTypeId,
									m_entResTypeId, na.getId(), null);
							addArc(p_nap2, t_natend, "PtT", entResTypeId, b_entResTypeId,
							...
							addArc(p_nap2, t_natstart, "TtP", entResTypeId, b_entResTypeId, null,
									addBtT(b_entResTypeId, entResTypeId, "(b.entity.get,b.resource.get)"),
									addAddedTime(b_entResTypeId, na.getProcessingTimeExpression()), "5", false,
									na.getId());
							String t_natend = addTransition("_natend", null, null, b_entResTypeId, e_entResTypeId,
									m_entResTypeId, na.getId(), null);
							addArc(p_nap2, t_natend, "PtT", entResTypeId, b_entResTypeId,
							...
							addArc(p_nap2, t_natstart, "TtP", entTypeId, b_entTypeId, null,
									addBtT(b_entTypeId, entTypeId, "b.entity.get"),
									addAddedTime(b_entTypeId, na.getProcessingTimeExpression()), "5", false,
									na.getId());
							String t_natend = addTransition("_natend", null, null, b_entTypeId, e_entTypeId,
									m_entTypeId, na.getId(), null);
							addArc(p_nap2, t_natend, "PtT", entTypeId, b_entTypeId,
				...
				if (n instanceof Branch) {
					...
					for (Connector c : p.getConnectors().values()) {
						if (b.getGate() == BranchGate.AND) {
							if (c.getTarget().getValue() != b)
								continue;
							String p_bpis = addPlace(b.getId() + "_end_" + c.getTargetIndex(), "_bpi" + i + "s",
									entTypeId, "", b.getId(), null);
							addArc(p_bpis, bt, "PtT", entTypeId, b_entTypeId,
									addTtB(b_entTypeId, entTypeId, "entitiy", "Some(entity)"),
									addBtT(b_entTypeId, entTypeId, "b.entity.get"), null, null, true, b.getId());
							...
							if (b.getType() == BranchType.SPLIT) {
								...
								String t_condGen = addTransition("_condGen", null, action, b_entBrTypeId, e_entBrTypeId,
										m_entBrTypeId, b.getId(), null);
								String p_bpt = addPlace(null, "_bpt", entBrTypeId, "", b.getId(), null);
								bp = p_bpt;
								addArc(p_bpis, t_condGen, "PtT", entTypeId, b_entBrTypeId,
										addTtB(b_entBrTypeId, entTypeId, "entity", "Some(entity),None"),
										addBtT(b_entBrTypeId, entTypeId, "b.entity.get"), null, null, true, b.getId());
								addArc(p_bpt, t_condGen, "TtP", entBrTypeId, b_entBrTypeId,
										addTtB(b_entBrTypeId, entBrTypeId, "(entity,cond)", "Some(entity),Some(cond)"),
										addBtT(b_entBrTypeId, entBrTypeId, "(b.entity.get,b.cond.get)"), null, null,
										false, b.getId());
							} else {
								if (c.getSource().getValue() != b)
									continue;
								String p_bpos = addPlace(b.getId() + "_start_" + c.getSourceIndex(), "_bpo" + i + "s",
										entTypeId, "", b.getId(), null);
								bp = p_bpos;
							}
						}
					...
					for (Connector c : p.getConnectors().values()) {
						if (b.getGate() == BranchGate.AND) {
							if (c.getSource().getValue() != b)
								continue;
							String p_bpos = addPlace(b.getId() + "_start_" + c.getSourceIndex(), "_bpo_" + i + "s",
									entTypeId, "", b.getId(), null);
							addArc(p_bpos, bt, "TtP", entTypeId, b_entTypeId, null,
									addBtT(b_entTypeId, entTypeId, "b.entity.get"), null, null, false, b.getId());
						} else if (b.getGate() == BranchGate.XOR) {
							if (b.getType() == BranchType.SPLIT) {
								if (c.getSource().getValue() != b)
									continue;
								...
								String p_bpos = addPlace(b.getId() + "_start_" + c.getSourceIndex(), "_bpo" + i + "s",
										entTypeId, "", b.getId(), null);
								String t_silent = addTransition("_temps_" + i, guard, null, b_entBrTypeId,
										e_entBrTypeId, m_entBrTypeId, b.getId(), null);
								addArc(bp, t_silent, "PtT", entBrTypeId, b_entBrTypeId,
										addTtB(b_entBrTypeId, entBrTypeId, "(entity,cond)", "Some(entity),Some(cond)"),
										addBtT(b_entBrTypeId, entBrTypeId, "(b.entity.get,b.cond.get)"), null, null,
										true, b.getId());
								addArc(p_bpos, t_silent, "TtP", entTypeId, b_entBrTypeId, null,
										addBtT(b_entBrTypeId, entTypeId, "b.entity.get"), null, null, false, b.getId());
							} else {
								if (c.getTarget().getValue() != b)
									continue;
								String p_bpis = addPlace(b.getId() + "_end_" + c.getTargetIndex(), "_bpi" + i + "s",
										entTypeId, "", b.getId(), null);
								String t_silent = addTransition("_temps_" + i, null, null, b_entTypeId, e_entTypeId,
										m_entTypeId, b.getId(), null);
								addArc(p_bpis, t_silent, "PtT", entTypeId, b_entTypeId,
										addTtB(b_entTypeId, entTypeId, "entity", "Some(entity)"),
										addBtT(b_entTypeId, entTypeId, "b.entity.get"), null, null, true, b.getId());
								addArc(bp, t_silent, "TtP", entTypeId, b_entTypeId, null,
										addBtT(b_entTypeId, entTypeId, "b.entity.get"), null, null, false, b.getId());
							}
						}
			...
			for (String s : p.getConnectors().keySet()) {
				...
				String[] oriRole = target.contains("-stop-") ? new String[] { target, "_stop" }
						: new String[] { c.getId(), "silent" };
				String t_silent = addTransition(oriRole[1], null, null, b_entTypeId, e_entTypeId, m_entTypeId,
						oriRole[0], null);
				addArc(source, t_silent, "PtT", entTypeId, b_entTypeId,
						addTtB(b_entTypeId, entTypeId, "entity", "Some(entity)"),
						addBtT(b_entTypeId, entTypeId, "b.entity.get"), null, null, true, c.getId());
				addArc(target, t_silent, "TtP", entTypeId, b_entTypeId, null,
						addBtT(b_entTypeId, entTypeId, "b.entity.get"), null, null, false, c.getId());
			}
		}
