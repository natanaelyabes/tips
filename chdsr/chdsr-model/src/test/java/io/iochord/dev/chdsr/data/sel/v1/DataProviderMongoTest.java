...
import io.iochord.dev.chdsr.data.sel.v1.impl.EventMongoImpl;
...
public class DataProviderMongoTest {
	...
	public void test01Insertion() throws Exception {
		...
		List<EventMongoImpl> el = new ArrayList<>();
		while (i < im) {
			for (String[] row : allData) {
				...
				String eo = row[8];
				String er = row[8];
				EventMongoImpl e = new EventMongoImpl(ec, ea, et, eo, er);
				getTemplate().save(e);
//				if (el.size() < 1000) {
//					el.add(e);
//				} else {
...
//				}
	...
	public void test02Selection() throws Exception {
		...
		Query q = new Query();
		q.with(s);
		Iterable<EventMongoImpl> it = getTemplate().find(q, EventMongoImpl.class);
